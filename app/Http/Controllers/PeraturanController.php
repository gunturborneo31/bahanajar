...
class PeraturanController extends Controller
{
    ...
    public function scrapeInaprocSections(Request $request): JsonResponse
    {
        ...
        $html_sections = '
        <ul>
            <li title="Produk Hukum">
                <a href="https://bantuan.inaproc.id/hc/id-id/categories/9038110434703-Produk-Hukum">Produk Hukum</a>
            </li>
            <li title="Peraturan">
                <a href="https://bantuan.inaproc.id/hc/id-id/sections/11107104099599-Peraturan">Peraturan</a>
            </li>
            <li title="Peraturan Pemerintah">
                <a href="https://bantuan.inaproc.id/hc/id-id/sections/14483994791823-Peraturan-Pemerintah">Peraturan Pemerintah</a>
            </li>
            <!-- BEBAS tambah li lain, title <a> tidak dipakai -->
        </ul>';
        ...
        foreach ($section_items as $li) {
            $jenis = trim($li->getAttribute('title'));  // title="Produk Hukum"
            $a = $li->getElementsByTagName('a')->item(0);
            $href = $a ? $a->getAttribute('href') : '';

            // 3. EXTRACT ID dari href (categories/sections/XXXXX)
            $section_id = null;
            ...
            if ($section_id) {
                // 4. SMART MATCHING articles (3 cara)
         ...
         public function updateInaproc3Jenis(Request $request): JsonResponse
    {
        ...
        // 1. BUAT 3 KOLOM via Schema::table PROPER (FIX SYNTAX)
        Schema::table('articles', function ($table) {
        ...
        // 2. 3 JENIS INAPROC + section_id
        $sections = [
            ['Produk Hukum', 9038110434703, 'categories'],
            ['Peraturan', 11107104099599, 'sections'],
            ['Peraturan Pemerintah', 14483994791823, 'sections']
        ];
    ...
    public function show($id)
    {
        ...
        return view('pages.peraturan-detail', compact('article'));
    }
}
